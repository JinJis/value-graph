...
async def render_pdf(html: str | None = None, doc_url: str | None = None, ua: str | None = None) -> bytes:
    """Normalize a filing document (HTML / DART markup) to PDF at INGEST time (one-shot).

    The heavy headless step is paid once per filing so query-time evidence highlighting can
    use fast PyMuPDF (no browser) on the cached PDF. Prefers caller-supplied `html`
    (datasets already fetched it with the right User-Agent), falling back to `goto(doc_url)`.
    Raises on failure (the caller maps that to 502)."""
